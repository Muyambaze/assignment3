...
with open("diary_output.txt", "w") as f:
    for j in range(5):
        f.write(f"\nEntry {j+1} - {date_str[j]}:\n")
        f.write(f"Mood: {mood_num[j]}/10\n")
        f.write(f"{entries[j]}\n")
        f.write(f"Word Count: {word_counts[j]}\n")

    f.write(f"\nAverage Mood: {average_mood:.2f}/10\n")
    f.write("="*20 + "LONGEST ENTRY" + "="*20 + "\n")
    f.write(f"Date: {date_str[longest_index]}\n")
    f.write(f"Word Count: {word_counts[longest_index]}\n")
    f.write(f"{entries[longest_index]}\n")
